...
deleteBeg = 2  # Deletes defined number of end points from data

w0 = 8.8 * (10 ** (-6))  # Pump Diameter in m
w1 = 5.0 * (10 ** (-6))  # Probe Diameter in m
...
x0 = np.array([35, 1 * (10 ** 7)], dtype=float)  # Initial guesses for unknown parameters

# Fitting function. Note that "cos" is NOT cosine. It's the unknown parameter(s). Python is 0 indexed, so cos[0] is the first unknown parameter
...
def fun(frequency, cos, wa):
    ...
    cv = [2.48 * (10 ** 6), 0, 3.03 * (10 ** 6)]  # Vector of volumetric heat capacities for each layer, which is the product of specific heat capacity (cp) and density (rho). Even layers are always 0 because they are interfaces.
    t = [81 * (10 ** (-9)), 0, 1]  # Thickness of each layer; even layers are always 0 because they are interfaces.
...
data = np.loadtxt(filename, delimiter='\t', skiprows=1, dtype=float)  # Loads the text file from Zurich; this is the signal file
...
dataREF = np.loadtxt(filename, delimiter='\t', skiprows=1, dtype=float)  # Loads the reference line file from data
# collected by Zurich (secondary photodetector)
freq = data[deleteBeg:-deleteEnd, 0]  # Pulls the measured frequency data
phi_data = data[deleteBeg:-deleteEnd, 4]  # Pulls the measured phase data
phi_ref = dataREF[deleteBeg:-deleteEnd, 4]  # Pulls the reference phase data
